...
property_list_t *read_user_property(dataset_list_t *dataset, const char* prop) {
	...
	/* PATCH (zfs-backup-service): OpenZFS 2.x declares the third argument
	 * of nvlist_lookup_string() as const char **, which GCC 14+ rejects
	 * with an error when a char ** is passed. */
	const char *strval;
