...
const char *get_vdev_type(nvlist_ptr nv) {
	/* PATCH (zfs-backup-service): const char * for OpenZFS 2.x / GCC 14+. */
...
nvlist_ptr go_zpool_search_import(libzfs_handle_ptr zfsh, int paths, char **path, boolean_t do_scan) {
	/*
	 * PATCH (zfs-backup-service): the legacy zpool_search_import() API
	 * (importargs_t + libzfs_config_ops) was removed from the OpenZFS 2.x
	 * libzfs, so the original implementation no longer compiles against
	 * modern OpenZFS. Pool import discovery is not used by
	 * zfs-backup-service; report no candidates instead.
	 */
	(void)zfsh;
	(void)paths;
	(void)path;
